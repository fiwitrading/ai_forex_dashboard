...
events = []
for entry in factory_feed.entries[:30]:
    title = entry.title
    if any(x in title.lower() for x in ["high impact", "medium impact"]):
        impact = "🔥 High Impact" if "high" in title.lower() else "⚠️ Medium Impact"
        events.append({
            "Event": title.replace("High Impact Expected", "").replace("Medium Impact Expected", ""),
            "Impact": impact,
...
# === FOOTER ===
st.markdown(
    f"<p style='text-align:center; color:gray; font-size:13px;'>Last updated: "
    f"{datetime.datetime.now().strftime('%H:%M:%S')} | Refresh every 10 min automatically</p>",
    unsafe_allow_html=True
)
